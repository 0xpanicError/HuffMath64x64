...
contract HuffConfig {
    ...
    function with_addr_constant(
        ...
        const_overrides.push(
            Constant(key, bytesToString(abi.encodePacked(value_)))
        );
        return this;
    }
    ...
    function deploy(string memory file) public payable returns (address) {
        ...
        string memory rand_bytes = bytes32ToString(
            keccak256(abi.encode(bytes32(retData)))
        );
